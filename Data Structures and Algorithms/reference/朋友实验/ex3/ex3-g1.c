/* L170300102 XUJUNHE GRAPH PROBLEM1*/
#include <stdio.h>
#include <stdlib.h>
#include <limits.h>
...
void printTrace(MarkVertex* mark_vertex,int index);
...
void initial(MarkVertex* mark_vertex,int n)
{
    ...
    for(i=0; i<n; i++)
    {
        mark_vertex[i].index = i + 1;    // differ every vertex.
...
void printAllMinDistance(vertex* table,int n)
{
    ...
    for(i=0; i<1; i++)
    {
        ...
        for(j=0; j<n; j++)
        {
            ...
            printf("s%d->s%d",i,j);                      // need modify
...
void calculateMinDistance(vertex* table,MarkVertex* mark_vertex,int index,int n)
{
    ...
    do
    {
        ...
        while(p->next!=NULL)
        {
            ...
            if(new_distance < mark_vertex[p->index-1].distance)
            {
                mark_vertex[p->index-1].distance = new_distance;
                mark_vertex[p->index-1].previous = mark_vertex[index].index;
            }
        }
...
void printTrace(MarkVertex* mark_vertex,int index)
{
    int previous = mark_vertex[index-1].previous;
    if(previous)
    {
